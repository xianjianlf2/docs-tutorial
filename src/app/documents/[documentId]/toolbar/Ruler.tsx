...
const RULER_WIDTH = 816; // 标尺总宽度
const MIN_MARGIN_GAP = 100; // 左右边距之间的最小间距
...
const Ruler = () => {
    ...
    const handleMouseDownLeft = (e: React.MouseEvent<HTMLDivElement>) => {
        ...
        // 初始化拖拽线位置
        if (rulerRef.current) {
            const container = rulerRef.current.querySelector('#ruler-container') as HTMLElement;
            if (container) {
    ...
    const handleDoubleClickLeft = () => {
        setLeftMargin(56); // 重置为默认值
    };

    const handleDoubleClickRight = () => {
        setRightMargin(56); // 重置为默认值
    };
    ...
    useEffect(() => {
        const handleMouseMove = (e: MouseEvent) => {
            ...
            if (isDraggingLeft) {
                // 使用 ref 中的最新值
                const maxLeftPosition = RULER_WIDTH - rightMarginRef.current - MIN_MARGIN_GAP;
                ...
                const maxRightMargin = RULER_WIDTH - (leftMarginRef.current + MIN_MARGIN_GAP);
                // 限制右边距在有效范围内，但不能小于左边距 + 最小间距
                const newRightPosition = Math.max(0, Math.min(rightPosition, maxRightMargin));
        ...
        const findEditorContainer = () => {
            ...
            if (proseMirror) {
                ...
                while (parent && !found) {
                    ...
                    if (width === '816px' || parent.querySelector('div[style*="816px"]')) {
                        // 查找包含这个元素的包装容器
                        let wrapper = parent;
                        while (wrapper && wrapper.parentElement) {
                            const wrapperStyle = getComputedStyle(wrapper.parentElement);
                            if (wrapperStyle.width.includes('816') || wrapper.offsetWidth === 816) {
                                editorContainerRef.current = wrapper.parentElement as HTMLDivElement;
                                found = true;
                                break;
                            }
        ...
        return createPortal(
            <div
                ...
                style={{
                    ...
                    width: '1px',
                    background: 'repeating-linear-gradient(to bottom, transparent, transparent 4px, #3b82f6 4px, #3b82f6 8px)',
                }}
            />,
            document.body
        );
